...
type ctx = {env: (string * inf_typ) list, aliases: (string * inf_typ) list}
...
fun inferPat (P.PatWildcard, _) = TyPatWildcard (InfTypUnbound (gensym (), ref NONE))
  ...
  | inferPat (P.PatId (b, lid), {env, aliases}) =
    (case List.find (fn (n, _) => n = (String.concatWith "." lid)) env of
...
and inferFunDecl (P.DeclFunNonfix [], _) = raise InferenceErr "Empty nonfix functions should be impossible"
    ...
    let val fty = InfTypUnbound (gensym (), ref NONE)
        val (f, fid, fsig) =
            let val typ = case typ of SOME typ => makeInfTyp typ
                                    | NONE => InfTypNever
                val pats = List.map (fn pat => inferPat (pat, ctx)) pats
                val patidstys = List.concat (List.map getPatIdsTyps pats)
  ...
  | inferFunDecl (P.DeclFunInfixMany ((patl, id, patr, pats, typ, exp)::fs), ctx as {env, aliases}) =
    let val fty = InfTypUnbound (gensym (), ref NONE)
        ...
        val fs = List.map (fn (patl, id, patr, pats, typ, exp) =>
                              let val typ = case typ of SOME typ => makeInfTyp typ
                                                      | NONE => InfTypNever
                                  val patl = inferPat (patl, ctx)
                                  val patr = inferPat (patr, ctx)
    ...
    in
        (TyDeclFunInfixMany (f::fs), (fid, fsig))
    end
...
and inferDecl (P.DeclVal (vars, vals), ctx) =
  ...
  | inferDecl (P.DeclDataTyp (data, typs), ctx as {env, aliases=aliases}) =
    let val data = List.map (fn (vars, id, cons) =>
        ...
        val ctx = {env=(List.concat (List.map (fn ([], id, cons) =>
